...
//-------------------------------------------------------------------------------------------------------
int voltToCounts (float volt, float Rpsu){		return (int)((1-((((float)volt/1.25)-1)*Rpsu/20000))*1023);	}
float countsToVolt (int counts, float Rpsu){	return 1.25*(1+20000*(1-(float)counts/1023)/Rpsu);			}

int scanFloatValue ( float Rpsu ){
	printf(" Enter the new voltage value (ej.: 10.83) -> ");
	int num = voltToCounts(atof(gets((char *)floatScanBuffer)), Rpsu);iprintf("\r\n");
	while (num < 0 || num > 1023){
		printf(" ERROR: a value between 1.26V and 32V must be given \n");
		printf(" Reenter the new voltage value (ej.: 5.42) -> ");
		num = voltToCounts(atof(gets((char *)floatScanBuffer)), Rpsu);iprintf("\r\n");
	}
	printf(" Setting closest value = %.2f V (n = %d)\n", countsToVolt(num, Rpsu), num);
	return num;
}
