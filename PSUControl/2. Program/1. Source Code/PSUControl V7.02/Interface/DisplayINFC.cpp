...
BYTE sniAlmStDisp[SnI_NUMBER][2][5];	// Alarm Status Display for voltage, filled with one of the Alarm messages below
BYTE sniValDisp[SnI_NUMBER][6][8];		// SnI values display, nominalVoltage, vOut for each
...
#define DETECTED_ERROR_DISP_INFC sniAlmStDisp[12][0][3]=(sniList[12].alarmProtocols[__(0, 2)]?ProtocolON_DISP:ProtocolOFF_DISP);
...
void DisplayINFC ( void ){
	...
	iprintf("-------------------------------------------------------------------------------\r\n" );
	iprintf(" (1) Display Status\r\n" );
	iprintf(" (2) Display PSU Configuration\r\n" );
	iprintf(" (3) Display SnI Configuration\r\n" );
...
void processCommandDisplayINFC ( void ){
	switch (categorySelectionINFC){
		case '1': statusDisplayINFC();  break;
		case '2': configDisplayINFC(PSU_TYPE_LIST);  break;
		case '3': configDisplayINFC(SnI_TYPE_LIST);  break;
...
void refreshValuesStatusINFC ( void ){
	...
	// PSU
	for (k=0; k<PSU_NUMBER; k++){
		mergeBuffer(psuValDisp[k][0],(BYTE*) ftos(psuList[k].progVolt, 3), 7);	fillWithBlank(psuValDisp[k][0],7); // progValue
	...
	// SnI
	for (k=0; k<SnI_NUMBER; k++){
		mergeBuffer(sniValDisp[k][0],(BYTE*) ftos(sniList[k].nominalVolt , 3), 7);	fillWithBlank(sniValDisp[k][0],7); 	// nominalVoltage
...
//============================== (2) CONFIGURATION DISPLAY METHODS   ====================================//
	...
	if(psu_sni==PSU_TYPE_LIST){// PSU
		...
		for (k=0; k<SnI_NUMBER; k++){
			for (j=0; j<ALARM_NUMBER-2; j++){ // Voltage alarms
				sniAlmStDisp[k][j][0]=(sniList[k].alarmWatch[j]?alarmConnected_DISP:alarmDisconnected_DISP);
				sniAlmStDisp[k][j][1]=(sniList[k].alarmProtocols[__(j, 0)]?ProtocolON_DISP:ProtocolOFF_DISP);
				sniAlmStDisp[k][j][2]=(sniList[k].alarmProtocols[__(j, 1)]?ProtocolON_DISP:ProtocolOFF_DISP);
				sniAlmStDisp[k][j][3]=(sniList[k].alarmProtocols[__(j, 2)]?ProtocolON_DISP:ProtocolOFF_DISP);
	...
	if(psu_sni==PSU_TYPE_LIST){// PSU
		...
		for (k=0; k<SnI_NUMBER; k++){
			for (j=0; j<ALARM_NUMBER-2; j++){ // All alarms
				mergeBuffer(sniValDisp[k][3*j],(BYTE*) ftos(sniList[k].alarmLimitValues[j] , 2), 6);		fillWithBlank(sniValDisp[k][3*j],6); 	// alarmLimitValue
				itoa(sniList[k].alarmLimitTimes[j], (char*) sniValDisp[k][3*j+1]);							fillWithBlank(sniValDisp[k][3*j+1],3); 	// alarmLimitTime
				snprintf((char*)sniValDisp[k][3*j+2], 3, "%x", sniList[k].alarmProtocolShutdown[j]);		fillWithBlank(psuValDisp[k][3*j+2],3); 	// Shutdown Protocol
			}
		}
	}
}
