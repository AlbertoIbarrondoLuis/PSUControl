...
extern BOOL sniSelectionList[SUP_NUMBER];		// Used for various functions where multiple sni selection is required.
		...
		for (k=0; k<SUP_NUMBER; k++){
			if ( Selection[k] ){
				for (g=0; g<ALARM_NUMBER-2; g++){
					if(alarmSelection[g]){
						supList[k].alarmCounters[g] = 0;
						supList[k].alarmStatus[g] = false;
						supList[k].alarmLimitReached[g]=false;
					}
				}
			}
		}
	...
	if (psu_sni==PSU_TYPE_LIST){
		...
		for (k=0; k<SUP_NUMBER; k++){
			if ( Selection[k] ){
				for (g=0; g<ALARM_NUMBER-2; g++){
					if(alarmSelection[g]){
						supList[k].alarmWatch[g]=almWatch;
					}
				}
			}
		}
...
void setAlarm ( BOOL psu_sni, int num, BOOL inf_sup, BOOL volt_curr, float value, int time){
	if(psu_sni==PSU_TYPE_LIST){
		...
		supList[num].alarmLimitTimes[_(inf_sup, volt_curr)]=time;
		supList[num].alarmLimitValues[_(inf_sup, volt_curr)]=value;
		supList[num].alarmWatch[_(inf_sup, volt_curr)]=true;
	}
}
...
void updateAlarms (void){
	...
	//Same procedure for supList, controlled by config_AlarmCTRLUpdate_SnI_FLAG (switched ON by default)
	if (config_AlarmCTRLUpdate_SnI_FLAG){
		for (NumAlm=0; NumAlm<SUP_NUMBER; NumAlm++){
...
void alarmCheck (int Num, BOOL inf_sup, BOOL volt_corr, BOOL psu_sni){
	...
	else{			// psu_sni == SnI_TYPE_LIST - Checking an SnI_TYPE from supList
		if(supList[Num].alarmWatch[_(inf_sup,volt_corr)]){				// If the selected alarm is being watched (INFERIOR/SUPERIOR, VOLTAGE/CURRENT)
																					// Refresh alarmLimitReached value
			if(inf_sup == INFERIOR){
				supList[Num].alarmLimitReached[_(inf_sup,volt_corr)]=(supList[Num].vOut <= supList[Num].alarmLimitValues[_(inf_sup,volt_corr)]);
			}
			else{
				supList[Num].alarmLimitReached[_(inf_sup,volt_corr)]=(supList[Num].vOut >= supList[Num].alarmLimitValues[_(inf_sup,volt_corr)]);
			}
			if (supList[Num].alarmLimitReached[_(inf_sup,volt_corr)]){			// If limit is exceeded
				supList[Num].alarmCounters[_(inf_sup,volt_corr)]++;					// Increment Alarm Counter

				if (supList[Num].alarmCounters[_(inf_sup,volt_corr)]>=supList[Num].alarmLimitTimes[_(inf_sup,volt_corr)]){
																						//  Trigger on the alarm if counter reaches the time limit
					if (supList[Num].alarmStatus[_(inf_sup,volt_corr)] == false ){			// If it's the first time the alarm is triggered, execute protocols
						supList[Num].alarmStatus[_(inf_sup,volt_corr)] = true;
						BYTE j;
						for (j=0; j<PROTOCOLS_NUMBER; j++){									// Goes over each alarm protocol for the activated alarm
							if (supList[Num].alarmProtocols[___(inf_sup,volt_corr, j)]){
								executeAlarmProtocol (Num, inf_sup,volt_corr, j, psu_sni);
							}
						}
					}
					supList[Num].alarmCounters[_(inf_sup,volt_corr)]=supList[Num].alarmLimitTimes[_(inf_sup,volt_corr)];
				}																		// Sets the counter to time limit (thus preventing it to increase further)
			}
			else{																	// If limit is not exceeded
				if (supList[Num].alarmCounters[_(inf_sup,volt_corr)] > 0){			// And counter is higher than 0
					supList[Num].alarmCounters[_(inf_sup,volt_corr)] -= 1;			// Decrease counter until it reaches 0

				}
				if (supList[Num].alarmCounters[_(inf_sup,volt_corr)] == 0){			// And when it hits 0, shut off the alarm.
					supList[Num].alarmStatus[_(inf_sup,volt_corr)] = false;
				}
			}
		}
		else{
			supList[Num].alarmCounters[_(inf_sup,volt_corr)] = 0;
			supList[Num].alarmStatus[_(inf_sup,volt_corr)] = false;
		}
	}
}
...
void executeAlarmProtocol (int Num, BOOL inf_sup, BOOL volt_corr, BYTE protocolNum, BOOL psu_sni){
	switch (protocolNum){		// (0) Shut down certain PSUs, (1) Modify Voltage, (2) Send Alarm Message.
		case PROTOCOL_SHUTDOWN:
			...
			else/*psu_sni==SnI_TYPE_LIST*/ 	{conversionShutdownWORDtoARRAY(supList[Num].alarmProtocolShutdown[_(inf_sup, volt_corr)], psuProtocolList);}
