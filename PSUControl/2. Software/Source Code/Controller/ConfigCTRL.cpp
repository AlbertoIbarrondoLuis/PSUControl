...
BOOL config_MonitorCTRL_PrintVals = CONFIG_INIT_MONITORCTRL_PRINT;				// Supply & Internal voltages																		// included in a2d loop or not
...
BOOL testMode_MonitorCTRL_Print_FLAG = false;				// Prints A2D read value when valid
...
//Periods & Flags
int adc_sampling_period = ADC_SAMPLING_PERIOD;
int monitor_print_period = MONITOR_PRINT_PERIOD;
BOOL monitor_print_FLAG = false;
int i2c_wait_period = I2C_WAIT_PERIOD;
...
//-------------------------------------------------------------------------------------------------------
// toggle_MonitorCTRL_PrintVals - Toggles between printing/not printing values every MONITOR_PRINT_PERIOD
//						in MonitorTask.
// 		#INPUT:	BOOL PrintValsFLAG - TRUE: print. FALSE: don't print.
// 		#OUTPUT: None
//		>E.G.: toggle_MonitorCTRL_PrintVals ( true );
//-------------------------------------------------------------------------------------------------------
void toggle_MonitorCTRL_PrintVals ( BOOL PrintValsFLAG )	{
	config_MonitorCTRL_PrintVals = PrintValsFLAG;
}
...
//-------------------------------------------------------------------------------------------------------
// config_i2cWait_I2CnSPI - Selects the wait period after sending an I2C Message
// 		#INPUT:	BOOL i2cWait - /Fsys to wait after sending I2c message
// 		#OUTPUT: None
//		>E.G.: config_i2cWait_I2CnSPI ( 10000 );
//-------------------------------------------------------------------------------------------------------
void config_i2cWait_I2CnSPI ( uint i2cWait )	{
	i2c_wait_period = i2cWait;
	iprintf("     #I2C wait: %d#\n", i2cWait);
}
...
// 		#OUTPUT: None
//		>E.G.: testMode_MonitorCTRL_Measure ( true );
//-------------------------------------------------------------------------------------------------------
...
//============================================TEST MODES===============================================//
//-------------------------------------------------------------------------------------------------------
// testMode_MonitorCTRL_Print - Prints valid values when stored in Monitor Task
// 		#INPUT:	BOOL testModeMonitorCTRLPrintFLAG - 	TRUE: Print
//														FALSE(default): don't print
// 		#OUTPUT: None
//		>E.G.: testMode_MonitorCTRL_Print ( true );
//-------------------------------------------------------------------------------------------------------
void testMode_MonitorCTRL_Print ( BOOL testModeMonitorCTRLPrintFLAG ){
	testMode_MonitorCTRL_Print_FLAG = testModeMonitorCTRLPrintFLAG;
}
