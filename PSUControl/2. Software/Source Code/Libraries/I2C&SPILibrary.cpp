...
//===========================================VARIABLES=================================================//
// Wait after each sent I2C message
extern int i2c_wait_period;
int i2c_wait_count = i2c_wait_period;
#define WAIT_I2C_LOOP {i2c_wait_count=i2c_wait_period;while(i2c_wait_count>0){i2c_wait_count--;}}
...
BYTE receiveBuffer[12];   // User created I2C output buffer
BYTE* pRecBuf = receiveBuffer;         	// Pointer to user I2C output buffer

//Array with indexed I2C addresses
...
BYTE sendI2CMessage ( BYTE outputBuffer[], uint bufSize, BYTE I2CAdress){
		...
		if( consoleOutputI2C )	{	if(I2CStat == I2C_OK)	{	iprintf( " -> I2C OK\n" );					}
									else 					{	iprintf( " -> I2C ERROR:%d\r\n", I2CStat);
																errorDetails(I2CStat);						}}

		iprintf(" \b \b \b \b \b \b \b \b \b \b \b \b \b \b \b \b \b \b");		// wait between I2C communications
...
BYTE readI2CBridgeBuffer ( BYTE inputBuffer[], uint bufSize, BYTE I2CtoSPIBridgeAddress){
	...
	if ( consoleOutputI2C )	{
		if(I2CStat == I2C_OK){	iprintf("Data Received -> " );printBuffer(inputBuffer, bufSize);
								iprintf(" -> I2C OK\r\n" );		}

		else{					iprintf("I2C ERROR: Failed to read due to error: %d\r\n", I2CStat);
								errorDetails(I2CStat);			}}
...
//-------------------------------------------------------------------------------------------------------
// addr2PSUnum - Converts from an I2C/SPI address to its correspondent PSU number
// 		#INPUT: BYTE BYTE I2CAddress - I2C address of the I2CtoSPI bridge.
//				BYTE SPIAddress - 0x1 or 0x2
// 		#OUTPUT: uint psuNum - number of the PSU related to this address
//		>E.G.: addr2PSUnum ( 0x2e, 0x1 ); - will return psuNum=2
//-------------------------------------------------------------------------------------------------------
int addr2PSUnum ( BYTE I2CAddress, BYTE SPIAddress ){
	int psuNum=0, i; BOOL isPSU=false;
	for ( i=0; i<PSU_NUMBER; i++ ){
		if ( i2CAddressArray[i] == I2CAddress ){
			psuNum = i;
			isPSU=true;
		}
	}
	psuNum = psuNum + SPIAddress - 2;
	return (isPSU?psuNum:-1);
}
